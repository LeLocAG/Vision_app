...
CONFIG_FILE = "api_key.txt" # File lưu key sẽ nằm cạnh file EXE
...
# --- 3. CẤU HÌNH ---
APP_TITLE = "Vision Translator Vibecoding Lê Lộc =)))"
APP_WIDTH = 720
APP_HEIGHT = 680
...
def smart_process_rotation(keys_input, image_path):
    # Lưu key ngay khi bắt đầu xử lý để lần sau không phải nhập lại
    save_keys_to_file(keys_input)
...
def fallback_google_translate(image_path):
    try:
        if not tess_path or not os.path.exists(tess_path):
            return "⚠️ CẢNH BÁO: Tất cả Key AI lỗi & Chưa cài Tesseract!"
        with Image.open(image_path) as img:
            raw_text = pytesseract.image_to_string(img, lang='eng')
        if not raw_text.strip(): return "⚠️ Ảnh không có chữ."
...
class SnippingTool:
    def __init__(self, master, callback):
        self.master = master; self.callback = callback; self.start_x = None; self.start_y = None; self.rect = None
        self.top = Toplevel(master); self.top.attributes("-fullscreen", True); self.top.attributes("-alpha", 0.3)
        self.top.attributes("-topmost", True)
        self.top.configure(bg="black", cursor="cross")
        self.canvas = Canvas(self.top, bg="black", highlightthickness=0)
        self.canvas.pack(fill="both", expand=True)
        self.canvas.bind("<ButtonPress-1>", self.on_press); self.canvas.bind("<B1-Motion>", self.on_move)
        self.canvas.bind("<ButtonRelease-1>", self.on_release); self.canvas.bind("<Escape>", self.on_cancel)
        self.master.withdraw()

    def on_press(self, event): self.start_x = event.x; self.start_y = event.y; self.rect = self.canvas.create_rectangle(self.start_x, self.start_y, 1, 1, outline=COLOR_BTN, width=2)
    def on_move(self, event): self.canvas.coords(self.rect, self.start_x, self.start_y, event.x, event.y)
    def on_cancel(self, event=None): self.top.destroy(); self.master.deiconify()
    def on_release(self, event):
        x1, y1, x2, y2 = min(self.start_x, event.x), min(self.start_y, event.y), max(self.start_x, event.x), max(self.start_y, event.y)
        self.top.destroy()
        if x2 - x1 < 10 or y2 - y1 < 10: self.master.deiconify(); return
    ...
    def capture_and_process(self, x1, y1, x2, y2):
        ...
        try:
            ...
            img.save(temp_file); self.callback(temp_file)
        except Exception as e: self.master.deiconify(); messagebox.showerror("Lỗi", str(e))

# --- 8. GIAO DIỆN CHÍNH ---
class App:
    def __init__(self, root):
        self.root = root; self.root.title(APP_TITLE); self.root.geometry(APP_SIZE)
        self.root.configure(bg=COLOR_BG); self.root.resizable(False, False)
        ...
        try:
            ...
            bg_img_pil = bg_img_pil.resize((APP_WIDTH, header_height), Image.LANCZOS)
        ...
        # INPUT KEY (AUTO LOAD)
        frame_key = tk.Frame(root, bg=COLOR_BG); frame_key.pack(fill="x", padx=20, pady=(15, 0))
        tk.Label(frame_key, text="Danh sách API Key (Tự động lưu khi chạy):", bg=COLOR_BG, fg=COLOR_TEXT, anchor="w").pack(fill="x")
        self.txt_keys = tk.Text(frame_key, height=3, bg="#2D2D2D", fg="white", insertbackground="white", relief="flat", font=("Consolas", 10))
        self.txt_keys.pack(fill="x", pady=5)
        
        # --- NẠP KEY ĐÃ LƯU ---
        saved_key = load_saved_keys()
        if saved_key:
            self.txt_keys.insert("1.0", saved_key)
        else:
            self.txt_keys.insert("1.0", "DÁN_KEY_VÀO_ĐÂY")
        ...
        self.txt_result.insert("1.0", "Xin chào người anh em  'api_key.txt'.\n hehe ")

        # BUTTON
        frame_btn = tk.Frame(root, bg=COLOR_BG); frame_btn.pack(fill="x", pady=(0, 20), padx=20)
        tk.Button(frame_btn, text="📸 CHỤP MÀN HÌNH (F2)", bg=COLOR_BTN, fg="white", font=("Segoe UI", 11, "bold"), pady=10, command=self.start_snip, relief="flat", cursor="hand2").pack(fill="x")

    def start_snip(self): self.root.after(0, lambda: SnippingTool(self.root, self.handle_image))
    
    def handle_image(self, image_path):
        self.root.deiconify()
        self.txt_result.delete("1.0", tk.END)
        self.txt_result.insert("1.0", "⏳ Đang xử lý...\n")
    ...
    def run_process(self, image_path):
        ...
        if "DÁN_KEY" in keys_text: keys_text = ""
        
        # Hàm này đã tích hợp lệnh Save Key
